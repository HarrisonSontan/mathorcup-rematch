...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes):
    ...
    for ind in range(len(gt_imgs)):
        ...
        # 计算单张图像分割结果为1的占总图像像素数量的百分比
        percentage = np.sum(pred == 1) / np.prod(pred.shape)
        file_name = os.path.splitext(os.path.basename(pred_imgs[ind]))[0] + ".jpg"

        precentage_list.append((file_name, percentage * 100))
    ...
    for ind_class in range(num_classes):
        print(
            ...
            + ":\tIou-"
            + str(round(IoUs[ind_class] * 100, 2))
    ...
    with open("./test.csv", mode="w", newline="") as file:
        ...
        writer.writerow(["fnames", "pothole %"])  # 写入表头
        for file_name, percentage in precentage_list:
            writer.writerow([file_name, "{:.0f}".format(percentage)])  # 写入每一行数据

    return np.array(hist, np.float64("inf")), IoUs, PA_Recall, Precision


def only_percentage(pred_dir, png_name_list):
    pred_imgs = [join(pred_dir, x + ".png") for x in png_name_list]
...
def draw_plot_func(
    ...
    fig = plt.gcf()
    axes = plt.gca()
    plt.barh(range(len(values)), values, color="royalblue", height=0.3)
...
def show_results(
    ...
    from utils_plot import Myplot as matrix_p

    # 计算tp、tn、fp、fn
    # tp = sum([hist[i][i] for i in range(len(hist))])
    # tn = sum([sum(hist[i]) for i in range(len(hist))]) - (
    #  sum([hist[i][i] for i in range(len(hist))])
    #  + sum([sum(hist[i]) for i in range(len(hist))])
    #  - sum([hist[i][i] for i in range(len(hist))])
    # )
    # fp = sum([sum(hist[i]) - hist[i][i] for i in range(len(hist))])
    # fn = sum([sum(hist[i]) - hist[i][i] for i in range(len(hist))])

    # print(hist)

    advanced_plt = matrix_p(2, 2)
    advanced_plt.plot_confusion_matrix(
        0, 0, hist[0][1], hist[0][0], hist[1][1], hist[1][0]
    )
    advanced_plt.fresh()
    advanced_plt.save(os.path.join(miou_out_path, "confusion_matrix.png"))
